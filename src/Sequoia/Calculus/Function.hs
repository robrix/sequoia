...
class Core k v s => FunctionIntro k v s where
  funL, (->⊢)
    :: (Pos a, Neg b)
    => _Γ -|s|- _Δ > a   ->   b < _Γ -|s|- _Δ
    -- --------------------------------------
...
funL2
  :: (FunctionIntro k v s, Pos a, Neg b)
  -- ---------------------------------
  => a ~~k~> b < a < _Γ -|s|- _Δ > b
funL2 = init ->⊢ init

($$)
  :: (Weaken k v s, Exchange k v s, FunctionIntro k v s, Pos a, Neg b)
...
funLPar
  :: (Weaken k v s, Exchange k v s, FunctionIntro k v s, ParIntro k v s, NotIntro k v s, Pos a, Neg b)
  =>  k ¬a ⅋ b < _Γ -|s|- _Δ
  -- -----------------------
  -> a ~~k~> b < _Γ -|s|- _Δ
